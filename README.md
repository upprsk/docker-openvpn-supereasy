...
Run an OpenVPN server with just one Docker command. No need to set up certs, CA etc. No need to run any command to create a client config file. Just run the container, specify how many clients you want and it will do everything by itself.
...
openvpn-supereasy:
  ...
  environment:
    ...
    - OPENVPN_PING_RESTART=60 #optional, restart connection if no ping has been successful for the specified time
    - OPENVPN_CIPHER=AES-256-GCM #optional, one of AES-256-GCM, AES-128-GCM, AES-256-CBC or AES-128-CBC. If omitted OpenVPN tries to negotiate the most secure cipher supported by both server and client
    - OPENVPN_FASTIO=0 #optional, enable fastio on the server
    - OPENVPN_NETWORK_UDP=10.8.0.0/24 #optional, network that will be used for clients connecting through udp. Defaults to 10.8.0.0/24
    - OPENVPN_NETWORK_TCP=10.9.0.0/24 #optional, network that will be used for clients connecting through tcp. Defaults to 10.9.0.0/24
...
The `.ovpn` files are re-generated every time the container starts. This allows to change parameters such as the server port, protocol, dns etc and still have the .ovpn file updated. The cert/key combination for each client will not change even if the `.ovpn` file is re-generated.
